...
import { Activity, BookOpen, Boxes, CircleDollarSign, Database, FolderGit2, Gauge, Search, Settings, Wrench } from 'lucide-react'
import './styles.css'

type Page = 'Overview'|'Live'|'Explain'|'Sessions'|'Tokens'|'Cost'|'Tools'|'MCP'
...
type Data = {overview:Overview;cost:Cost;sessions:Session[];briefings:Briefing[];explanations:Briefing[];tools:ActivityData;mcp:ActivityData;series:Point[];projects:Breakdown[];models:Breakdown[];costProjects:Breakdown[];costModels:Breakdown[];costSessions:Breakdown[]}
const Charts = React.lazy(()=>import('./charts'))

const nav = [
  ['Overview', Gauge], ['Live', Activity], ['Explain', BookOpen], ['Projects', FolderGit2], ['Sessions', Database],
  ['Tokens', Boxes], ['Cost', CircleDollarSign], ['Models', Boxes], ['Tools', Wrench],
  ['MCP', Boxes], ['Git', FolderGit2], ['Search', Search], ['Settings', Settings],
] as const
const ready = new Set(['Overview','Live','Explain','Sessions','Tokens','Cost','Tools','MCP'])
...
function App(){
  ...
  React.useEffect(()=>{const loadAll=()=>Promise.all([
    ...
    api<Briefing[]>('/api/live/briefings'),api<Briefing[]>('/api/explain?limit=12'),api<ActivityData>('/api/tools'),api<ActivityData>('/api/mcp'),
  ...
  ]).then(([overview,cost,sessions,briefings,explanations,tools,mcp,series,projects,models,costProjects,costModels,costSessions])=>{setData({overview,cost,sessions,briefings,explanations,tools,mcp,series,projects,models,costProjects,costModels,costSessions});setError('')}).catch(()=>setError('The local analytics API is unavailable.'))
